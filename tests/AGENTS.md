...
## OVERVIEW
Tests are layered by scope; each layer maps to a pytest marker defined in pyproject `[tool.pytest.ini_options]`. The directory IS the marker: a test's location decides which `-m` flag runs it, CI runs the union. 5 layers, 7 capability packages.
