...
## OVERVIEW
The deepest directory in the repo (133 py files): 9 shipped capability packages (Country, Currency, Date, Email, IP, ISBN, Money, Phone, URL), each an independent recognize→validate→resolve mini-system wired into the shared pipeline via `paxman.core`. Each package is self-contained: grammars recognize representations, rules assign meaning with provenance, the contract selects what runs, and `format_value()` renders the result. A 10th — SI Unit (BIPM SI Brochure) — is in development on `feature/si-unit-capability`; update counts and the export list when it lands.
...
## NOTES
- `__init__.py` exports all nine shipped capabilities; completeness is enforced by `tests/unit/test_capability_exports.py`. The in-development SI Unit will be the tenth (update `__all__` and this count).
