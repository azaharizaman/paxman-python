...
|-----|---------------------|
| [Getting Started](getting-started.md) | Install with `pip` or `uv`, run your first call in a script or notebook |
| [Concepts](concepts/index.md) | The mental model — capabilities, contracts, pipeline, results, provenance |
| [Capabilities](capabilities/index.md) | Per-capability guides — one page per kind of identifier |
...
The [Concepts hub](concepts/index.md) is the place to build your mental model before diving into reference docs. Each page is self-contained and includes a Mermaid diagram:

- [Capabilities](concepts/capabilities.md) — what a capability is and how it is chosen
- [Contracts](concepts/contracts.md) — how you configure what Paxman recognizes and validates
- [Pipeline](concepts/pipeline.md) — what happens inside a `canonicalize()` call
- [Execution Result](concepts/execution-result.md) — how to read `status`, `canonicalized_value`, `span`, and `candidates`
- [Provenance](concepts/provenance.md) — what "authoritative" means and how to cite it
- [Candidates & Ambiguity](concepts/candidates-and-ambiguity.md) — why two answers can be correct
- [Errors](concepts/errors.md) — what raises an exception vs what returns a status

### Capability guides

Each guide under [Capabilities](capabilities/index.md) is also self-contained: what it recognizes, canonical output & `output_format`, contract flags, status examples, notebook snippet, and provenance.
